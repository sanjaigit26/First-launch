...
function initEarlyAccessForm() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        const message = data?.details?.[0]?.message || data?.error || 'Registration failed.';
      ...
      triggerConfetti();
      form.reset();
      setFeedback(data?.message || 'Registration saved.', 'success');
      if (typeof window.refreshEntriesDashboard === 'function') {
        window.refreshEntriesDashboard();
      }
    } catch (error) {
      console.error('Early access form submission failed:', error);
      setFeedback('Unable to reach the server. Please try again.', 'error');
    } finally {
